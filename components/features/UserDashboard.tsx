...
export default function UserDashboard({ user, profile, onSignOut }: { user: any, profile: Profile, onSignOut: () => void }) {
  ...
  const {
    ...
    selectedReasons, activeReasonsList, displayGreetingName, toggleReason, handleSubmit, resetSuccess
  } = useCheckIn(user, profile)

  // === SECURITY CHECK: Can this person see the Staff button? ===
  const canRegisterAsStaff = profile?.role === 'admin' || profile?.role === 'superadmin'
    ...
    return (
      <div className="w-full max-w-md p-8 bg-white dark:bg-gray-900 border-2 border-red-500 rounded-xl text-center animate-in fade-in zoom-in duration-300 shadow-2xl relative z-10 flex flex-col items-center">
        <div className="w-20 h-20 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center mb-6 border border-red-200 dark:border-red-800">
          <svg className="w-10 h-10 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
        </div>
        <h2 className="text-3xl font-extrabold text-red-600 dark:text-red-400 mb-2">Access Denied</h2>
        <p className="text-gray-600 dark:text-gray-300 mb-8 font-medium">Your account has been restricted by a Library Administrator.</p>
  ...
  if (hasVisitedToday && !showSuccess) {
    return (
      <div className="w-full max-w-md p-8 bg-white dark:bg-gray-900 border border-blue-200 dark:border-blue-900/50 rounded-xl text-center animate-in fade-in zoom-in duration-300 shadow-2xl relative z-10 flex flex-col items-center">
        <div className="w-16 h-16 bg-blue-50 dark:bg-blue-900/20 rounded-full flex items-center justify-center mb-4"><span className="text-3xl">📅</span></div>
        <h2 className="text-2xl font-bold text-black dark:text-white mb-2">Already Checked In!</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-8">It looks like you have already logged a visit to the library today, <span className="font-semibold text-black dark:text-white">{displayGreetingName}</span>.</p>
        <button onClick={() => setHasVisitedToday(false)} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-4 rounded-lg transition-all shadow-sm active:scale-[0.98] mb-4">Submit Another Entry</button>
  ...
  if (showSuccess) {
    return (
      <div className="w-full max-w-md p-8 bg-white dark:bg-gray-900 border border-green-500 rounded-xl text-center animate-in fade-in zoom-in duration-300 shadow-2xl relative z-10 flex flex-col items-center">
        <div className="w-16 h-16 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center mb-4"><svg className="w-8 h-8 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg></div>
        <h2 className="text-3xl font-bold text-black dark:text-white mb-2">Welcome to NEU Library!</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-8">Your visit has been recorded successfully, {displayGreetingName}.</p>
        <LogoutButton onSignOut={onSignOut} className="mb-4" />
        <button onClick={resetSuccess} className="text-sm text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white underline transition-colors">Submit another entry</button>
      </div>
    )
  }
  ...
  if (isFirstTime && !newUserType) {
    return (
      <div className="w-full max-w-md p-8 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-2xl relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-500 text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-6">Before we continue, please confirm how you are accessing the library today.</p>
        ...
        <div className="flex flex-col gap-4 mb-8">
          <label className={`cursor-pointer p-5 border-2 rounded-xl transition-all flex items-center gap-4 text-left ${selectedRole === 'student' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-500/50'}`}>
            <input type="radio" name="user_role" value="student" checked={selectedRole === 'student'} onChange={() => setSelectedRole('student')} className="w-5 h-5 text-blue-600 focus:ring-blue-500" />
          ...
          {/* NEW: This completely hides the Staff option from regular users */}
          {canRegisterAsStaff && (
            <label className={`cursor-pointer p-5 border-2 rounded-xl transition-all flex items-center gap-4 text-left ${selectedRole === 'staff' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-500/50'}`}>
              <input type="radio" name="user_role" value="staff" checked={selectedRole === 'staff'} onChange={() => setSelectedRole('staff')} className="w-5 h-5 text-blue-600 focus:ring-blue-500" />
              <div className="flex flex-col flex-1">
                <span className="font-bold text-gray-900 dark:text-white text-lg">Faculty / Staff</span>
        ...
        <button disabled={!selectedRole} onClick={() => setNewUserType(selectedRole)} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 dark:disabled:bg-gray-700 disabled:cursor-not-allowed text-white font-bold py-3.5 px-4 rounded-lg transition-all shadow-sm active:scale-[0.98]">Continue</button>
      </div>
    )
  }
  ...
  return (
    <div className={`w-full p-8 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-2xl relative z-10 transition-all duration-500 ease-in-out overflow-hidden ${isFirstTime ? 'max-w-4xl' : 'max-w-md'}`}>
      ...
      <div className="mb-8 text-center border-b border-gray-100 dark:border-gray-800 pb-6 relative">
        
        {isFirstTime && (
          <button 
            type="button" 
            onClick={() => setNewUserType(null)} 
            className="absolute left-0 top-1 flex items-center gap-1.5 text-sm font-bold text-gray-400 hover:text-blue-600 dark:text-gray-500 dark:hover:text-blue-400 transition-colors"
          >
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-3">
          {isFirstTime ? 'Please complete your registration and log your visit.' : 'Please log your visit below.'}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8">
              ...
              {activeUserType === 'student' && (
                <div>
                  ...
                  <input 
                    ...
                    type="text" 
                    value={schoolId} 
                    onChange={(e) => setSchoolId(e.target.value.replace(/[^0-9-]/g, ''))} 
                    placeholder="e.g. 01-2345-6789" 
                    className="w-full p-3.5 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-black dark:text-white focus:ring-2 focus:ring-blue-500 focus:outline-none" 
                  />
                </div>
              )}
        ...
        <button type="submit" disabled={selectedReasons.length === 0 || isSubmitting} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 dark:disabled:bg-gray-700 text-white font-bold py-4 px-4 rounded-lg shadow-lg shadow-blue-500/30 transition-all active:scale-[0.98] mt-2">
